...
def _desired_env_text(client, name: str, endpoint: str | None, env_file: Path | None) -> str:
    if env_file is not None:
        return _read_text(env_file)
    try:
        stack = client.get_stack(name, endpoint=endpoint)
    except NotFoundError:
        return ""
    return _stack_text(stack, ENV_KEYS)
...
def _assert_env_persisted(client, name: str, desired_env: str, endpoint: str | None) -> dict[str, Any]:
    stack = client.get_stack(name, endpoint=endpoint)
    current_env = _stack_text(stack, ENV_KEYS)
    if current_env != desired_env:
        raise ApiError("Dockge did not persist the submitted stack .env; getStack composeENV differs from --env-file")
    return stack
...
def _verify_stack(client, name: str, endpoint: str | None, health_url: str | None, timeout: float, interval: float, require_services: bool = True, desired_env: str | None = None) -> dict[str, Any]:
    ...
    while True:
        try:
            ...
            env_ok = desired_env is None or _stack_text(stack, ENV_KEYS) == desired_env
            stack_ok = bool(stack)
            ok = stack_ok and services_ok and health_ok and env_ok
            last = {"ok": ok, "stack": stack, "services": services, "health": health, "require_services": require_services, "env_ok": env_ok}
...
def plan_stack(
    ...
    try:
        ...
        plan = _build_plan(
            client,
            name,
            _read_text(compose_file),
            _desired_env_text(client, name, ep, env_file),
            ep,
        )
...
def diff_stack(
    ...
    try:
        ...
        env_text = _read_text(env_file, _stack_text(stack, ENV_KEYS))
...
def save_stack(
    ...
    try:
        ...
        _assert_env_persisted(client, name, env_text, ep)
